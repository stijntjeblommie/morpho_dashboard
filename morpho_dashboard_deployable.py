...
def load_json_from_file_or_upload(file_path: str, upload_widget, default_data: Dict) -> Dict:
    """Load JSON from local file or uploaded file"""
    if upload_widget is not None:
        try:
            return json.load(upload_widget)
        except Exception as e:
            st.error(f"Error loading uploaded file: {e}")
            return default_data

    # Try local file
    for possible_path in [file_path, f"data/{file_path}", f"./{file_path}"]:
        if os.path.exists(possible_path):
            try:
                with open(possible_path, 'r') as f:
                    return json.load(f)
            except Exception as e:
                st.warning(f"Error loading {possible_path}: {e}")
                continue
...
def process_morpho_data(morpho_data: Dict, pendle_summary: Dict, pendle_analysis: Dict, config: Dict = None) -> pd.DataFrame:
    ...
    for market_entry in morpho_data.get('data', []):
        ...
        if is_pendle_pt:
            ...
            pendle_data = get_pendle_data_for_market(market.get('uniqueKey', ''), pendle_analysis)
            if pendle_data and pendle_market:
                pendle_market = pendle_data.get('pendleMarket', {})
...
def load_data_sources():
    ...
    if local_files_exist:
        ...
        st.sidebar.warning("⚠️ No local files found")
        data_source = "upload"

    # Allow user to override
    source_override = st.sidebar.radio(
        "Data source:",
        ["Auto-detect", "Upload files"],
        index=0 if data_source != "upload" else 1
    )

    if source_override == "Upload files" or data_source == "upload":
        ...
        # Load from uploads
        morpho_data = load_json_from_file_or_upload("", morpho_file, {"data": []})
        pendle_summary = load_json_from_file_or_upload("", pendle_summary_file, {"ptMarkets": []})
        pendle_analysis = load_json_from_file_or_upload("", pendle_analysis_file, {"ptMarketsData": {}})

    elif data_source == "data_folder":
        # Load from data folder
        morpho_data = load_json_from_file_or_upload("data/morpho_complete_analysis.json", None, {"data": []})
        pendle_summary = load_json_from_file_or_upload("data/pendle_morpho_summary.json", None, {"ptMarkets": []})
        pendle_analysis = load_json_from_file_or_upload("data/pendle_morpho_analysis.json", None, {"ptMarketsData": {}})

    else:
        # Load from local files
        morpho_data = load_json_from_file_or_upload("morpho_complete_analysis.json", None, {"data": []})
        pendle_summary = load_json_from_file_or_upload("pendle_morpho_summary.json", None, {"ptMarkets": []})
        pendle_analysis = load_json_from_file_or_upload("pendle_morpho_analysis.json", None, {"ptMarketsData": {}})
...
def main():
    ...
    # Load data with flexible sources
    with st.spinner("Loading data..."):
        morpho_data, pendle_summary, pendle_analysis = load_data_sources()

    if not morpho_data.get('data'):
        st.error("❌ No Morpho data available.")
        st.info("💡 Please upload your JSON files using the sidebar, or ensure they exist in the project directory.")
        st.markdown("""
        **Required files:**
        - `morpho_complete_analysis.json` (from `node get_morpho_data.js`)
        - `pendle_morpho_summary.json` (from `node pendle_morpho_analyzer.js`)
        - `pendle_morpho_analysis.json` (from `node pendle_morpho_analyzer.js`)
        """)
        return
